...
Item {
  ...
  // Plain properties, not `required` ones: the host mounts a plugin bar with
  // Loader.source, which cannot initialize required properties, so declaring
  // them required makes the Loader fail before configureBar() ever injects
  // them. Defaults just have to survive until that call lands.
  //
  ...
  // Extra [bar] keys read straight off the merged shell.toml dict. Style only
  // whitelists the size/scale keys into barOverrides, so these ride on
  // Color.shellValues, which still carries every parsed section.key.
  function barStyleNum(key, fallback) {
    var n = Number(Color.shellValues["bar." + key])
    return isFinite(n) ? n : fallback
  }
  function barStylePx(key, fallback) {
    return Math.max(0, Math.round(barStyleNum(key, fallback)))
  }
  readonly property bool islands: Style.boolToken(Color.shellValues["bar.islands"], false)

  // `margin` sets all four sides; each side key overrides it on its own.
  readonly property int outerMargin: barStylePx("margin", 0)
  readonly property int marginTop: barStylePx("margin-top", outerMargin)
  readonly property int marginBottom: barStylePx("margin-bottom", outerMargin)
  readonly property int marginLeft: barStylePx("margin-left", outerMargin)
  readonly property int marginRight: barStylePx("margin-right", outerMargin)

  // Padding is screen-space, not bar-relative: x is always horizontal. On a
  // top/bottom bar that makes x the along-axis pad and y the thickness pad;
  // on a left/right bar the two swap roles.
  readonly property int islandPadding: barStylePx("island-padding", 8)
  readonly property int islandPaddingX: barStylePx("island-padding-x", islandPadding)
  readonly property int islandPaddingY: barStylePx("island-padding-y", islandPadding)
  readonly property int islandPaddingAlong: vertical ? islandPaddingY : islandPaddingX
  readonly property int islandPaddingAcross: vertical ? islandPaddingX : islandPaddingY
  readonly property int islandRadius: barStylePx("island-radius", Style.cornerRadius)

  // barSize stays the content strip; the across-axis padding grows the island
  // around it. The layer surface reserves that plus its margins, so windows
  // tile below the gap instead of under it.
  readonly property int islandThickness: barSize + (islands ? islandPaddingAcross * 2 : 0)
  readonly property int barExtent: islandThickness + (vertical ? marginLeft + marginRight : marginTop + marginBottom)
  readonly property int sectionInset: islands ? islandPaddingAlong : Style.space(8)
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.barExtent : 0
      bottom: root.barHidden && root.position === "bottom" ? -root.barExtent : 0
      left: root.barHidden && root.position === "left" ? -root.barExtent : 0
      right: root.barHidden && root.position === "right" ? -root.barExtent : 0
    }
    ...
    implicitWidth: root.vertical ? root.barExtent : 0
    implicitHeight: root.vertical ? 0 : root.barExtent
    // In island mode the sections paint their own background, so the strip
    // itself stays clear and the outer margin reads as a real gap.
    color: root.transparent || root.islands ? "transparent" : root.background
    ...
    Component {
      ...
      Item {
        ...
        Island { target: leftSection }
        LeftModules {
          id: leftSection
          anchors.left: parent.left
          anchors.leftMargin: root.sectionInset
          anchors.verticalCenter: parent.verticalCenter
        }

        Island { target: rightSection }
        RightModules {
          id: rightSection
          anchors.right: parent.right
          anchors.rightMargin: root.sectionInset
          anchors.verticalCenter: parent.verticalCenter
        }
      }
    }
    ...
    Component {
      ...
      Item {
        ...
        Island { target: topSection }
        LeftModules {
          id: topSection
          anchors.top: parent.top
          anchors.topMargin: root.sectionInset
          anchors.horizontalCenter: parent.horizontalCenter
        }

        Island { target: bottomSection }
        RightModules {
          id: bottomSection
          anchors.bottom: parent.bottom
          anchors.bottomMargin: root.sectionInset
          anchors.horizontalCenter: parent.horizontalCenter
        }
      }
    }
  ...
  // Background slab behind one bar section. Fills the strip across the bar and
  // hugs its section along it, so it can never spill into the outer margin.
  component Island: Rectangle {
    required property var target

    z: -1
    visible: root.islands && !!target && target.width > 0 && target.height > 0
    color: root.transparent ? "transparent" : root.background
    radius: Math.min(root.islandRadius, Math.min(width, height) / 2)
    anchors.centerIn: target
    width: root.vertical ? root.islandThickness : (target ? target.width : 0) + root.islandPaddingAlong * 2
    height: root.vertical ? (target ? target.height : 0) + root.islandPaddingAlong * 2 : root.islandThickness
  ...
  component CenterModules: Item {
    ...
    Component {
      ...
      Item {
        ...
        // The anchored arrangement lays three items out around the exact
        // center, so the island spans their combined extent rather than
        // wrapping any single one.
        readonly property real contentLeft: centerRoot.hasAnchor
          ? (centerBefore.width > 0 ? Math.min(centerAnchorModule.x, centerBefore.x) : centerAnchorModule.x)
          : centerFull.x
        readonly property real contentRight: centerRoot.hasAnchor
          ? (centerAfter.width > 0 ? Math.max(centerAnchorModule.x + centerAnchorModule.width, centerAfter.x + centerAfter.width) : centerAnchorModule.x + centerAnchorModule.width)
          : centerFull.x + centerFull.width
        ...
        Rectangle {
          z: -1
          visible: root.islands && parent.contentRight > parent.contentLeft
          color: root.transparent ? "transparent" : root.background
          radius: Math.min(root.islandRadius, Math.min(width, height) / 2)
          anchors.verticalCenter: parent.verticalCenter
          height: root.islandThickness
          x: parent.contentLeft - root.islandPaddingAlong
          width: parent.contentRight - parent.contentLeft + root.islandPaddingAlong * 2
    ...
    Component {
      ...
      Item {
        ...
        readonly property real contentTop: centerRoot.hasAnchor
          ? (centerBefore.height > 0 ? Math.min(centerAnchorModule.y, centerBefore.y) : centerAnchorModule.y)
          : centerFull.y
        readonly property real contentBottom: centerRoot.hasAnchor
          ? (centerAfter.height > 0 ? Math.max(centerAnchorModule.y + centerAnchorModule.height, centerAfter.y + centerAfter.height) : centerAnchorModule.y + centerAnchorModule.height)
          : centerFull.y + centerFull.height
        ...
        Rectangle {
          z: -1
          visible: root.islands && parent.contentBottom > parent.contentTop
          color: root.transparent ? "transparent" : root.background
          radius: Math.min(root.islandRadius, Math.min(width, height) / 2)
          anchors.horizontalCenter: parent.horizontalCenter
          width: root.islandThickness
          y: parent.contentTop - root.islandPaddingAlong
          height: parent.contentBottom - parent.contentTop + root.islandPaddingAlong * 2
